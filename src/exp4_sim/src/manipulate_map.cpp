...
void ModifyMap::ReceiveMapCallback(const grid_map_msgs::GridMap msg)
{
  ...
  if(first_time_){
    ...
    map_manipulated_.setFrameId(map_received_.getFrameId());
  ...
  float camera_angle_rad = 0.5;
  float correction_right = 0;
  float correction_left = 0.1;
  float d_min = 0.6;
  float d_max = 3;

  // Calculation of a point d_min(m) away from the robot's orientation in the direction of orientation +- alpha
  float pos_x_plus = pose_robot_.pose.position.x + cos(a_rad+camera_angle_rad+correction_right)*d_min;
  float pos_x_minus = pose_robot_.pose.position.x + cos(a_rad-camera_angle_rad-correction_left)*d_min;
  float pos_y_plus = pose_robot_.pose.position.y + sin(a_rad+camera_angle_rad+correction_right)*d_min;
  float pos_y_minus = pose_robot_.pose.position.y + sin(a_rad-camera_angle_rad-correction_left)*d_min;
  ...
  pos_x_plus = pose_robot_.pose.position.x + cos(a_rad+camera_angle_rad+correction_right)*d_max;
  pos_x_minus = pose_robot_.pose.position.x + cos(a_rad-camera_angle_rad-correction_left)*d_max;
  pos_y_plus = pose_robot_.pose.position.y + sin(a_rad+camera_angle_rad+correction_right)*d_max;
  pos_y_minus = pose_robot_.pose.position.y + sin(a_rad-camera_angle_rad-correction_left)*d_max;
  ...
  if(!map_manipulated_.isInside(pos_point2_x_y_plus)){
    ...
    for(float i = d_max-0.1; i >= d_min; i = i-0.1){
      pos_x_plus = pose_robot_.pose.position.x + cos(a_rad+camera_angle_rad)*i;
      pos_y_plus = pose_robot_.pose.position.y + sin(a_rad+camera_angle_rad)*i;
  ...
  if(!map_manipulated_.isInside(pos_point2_x_y_minus)){
    ...
    for(float i = d_max-0.1; i >= d_min; i = i-0.1){
      pos_x_minus = pose_robot_.pose.position.x + cos(a_rad-camera_angle_rad)*i;
      pos_y_minus = pose_robot_.pose.position.y + sin(a_rad-camera_angle_rad)*i;
      Position pos_point2_new_minus(pos_x_minus,pos_y_minus);
      if(map_manipulated_.isInside(pos_point2_new_minus)){
        pos_point2_x_y_minus = pos_point2_new_minus;
        break;
      }
    }
  }
  ...
  if(map_manipulated_.isInside(pos_point1_x_y_plus) && map_manipulated_.isInside(pos_point2_x_y_plus) && 
    ...
    for (grid_map::PolygonIterator it(map_manipulated_, polygon);
        !it.isPastEnd(); ++it) {
      if(!( map_manipulated_.at("elevation", *it) > -100)){    
  ...
  for (GridMapIterator it(map_manipulated_); !it.isPastEnd(); ++it) {
    ...
    }else if( !(map_manipulated_.at("elevation", *it)>-100) && (map_manipulated_.at("ground_manipulated", *it)>-100)){
